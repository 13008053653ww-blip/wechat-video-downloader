...
export function isCommand(text: string): boolean {
  ...
  return (
    HELP_KEYWORDS.some((kw) => kw.toLowerCase() === normalized) ||
 ...
 */
export async function handleCommand(text: string, openId: string): Promise<string> {
  const normalized = text.trim().toLowerCase();
  ...
  if (HISTORY_KEYWORDS.some((kw) => kw.toLowerCase() === normalized)) {
    ...
    return formatHistoryReply(records);
  }
