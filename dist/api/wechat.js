...
async function handler(req, res) {
    try {
        ...
        if (req.method === 'POST') {
            ...
            const message = (0, xmlParser_1.parseXml)(xmlBody);
            // 使用 Promise.race 实现 4.5 秒超时
            const replyText = await Promise.race([
                Promise.resolve((0, messageRouter_1.routeMessage)(message)),
                new Promise((resolve) => setTimeout(() => resolve('解析超时，请稍后重试'), TIMEOUT_MS)),
            ]);
            ...
            const xmlReply = (0, xmlParser_1.buildTextReply)(message.ToUserName, message.FromUserName, replyText);
