...
exports.isKvAvailable = isKvAvailable;
exports.addHistory = addHistory;
exports.getHistory = getHistory;
exports.formatHistoryReply = formatHistoryReply;
/** 每个用户最多保存的历史记录条数 */
 ...
 * - KV 不可用时静默返回
 */
async function addHistory(openId, record) {
    if (!isKvAvailable())
        return;
    try {
 ...
 * KV 不可用时返回空数组
 */
async function getHistory(openId, limit = DEFAULT_LIMIT) {
    if (!isKvAvailable())
        return [];
    try {
        const key = `history:${openId}`;
 ...
 * - 空记录返回"暂无解析记录"
 */
function formatHistoryReply(records) {
